...
## 13) Дизайн-система: Liquid Glass (v2.0)
...
Интерфейс полностью переработан в стиле Apple Liquid Glass — дизайн-языка, представленного на WWDC 2025 для iOS 26.
Ключевые принципы:
- **Белый минималистичный фон** с мягким градиентом.
- **Стеклянные поверхности (glass panels)** с `backdrop-filter: blur()`, полупрозрачными фонами и тонкими границами.
- **Плавные анимации** с cubic-bezier/spring-based easing.
- **Вертикальный mobile-first макет**, оптимизированный для экранов телефонов (320–430px).
- **Скруглённые концентрические формы**, связанные с геометрией устройства.
- **Минимум визуального шума**: контент первичен, UI вторичен.

### 13.2 Цветовая палитра

```
--glass-bg: rgba(255, 255, 255, 0.72)         /* Основной стеклянный фон */
--glass-bg-hover: rgba(255, 255, 255, 0.85)    /* Hover-состояние */
--glass-border: rgba(255, 255, 255, 0.5)       /* Светлая граница */
--glass-shadow: 0 8px 32px rgba(0, 0, 0, 0.08) /* Мягкая тень */
--accent: #34C759                                /* Зелёный акцент (iOS green) */
--accent-strong: #248A3D                         /* Тёмный зелёный */
--text-primary: #1C1C1E                          /* Основной текст */
--text-secondary: #8E8E93                        /* Вторичный текст */
--danger: #FF3B30                                /* Опасное действие */
--page-bg: linear-gradient(180deg, #F2F2F7 0%, #FFFFFF 50%, #F2F2F7 100%)
```

### 13.3 Типографика

- Шрифт: **Inter** (Google Fonts) — нейтральный, отличная читаемость на стекле.
- Заголовки: 600–700 weight, увеличенный letter-spacing (+0.02em).
- Тело текста: 400–500 weight.
- На стеклянных поверхностях: увеличенный font-weight (+100) для контрастности.
- Минимальный контраст: 4.5:1 против blur-фона.
...
- Внутренняя тень: `inset 0 1px 0 rgba(255, 255, 255, 0.8)` для спекулярного блика.

#### Habit Card
- Glass panel меньшего размера внутри списка.
- Мягкая анимация появления (fade-in + slide-up).
- Чекбокс: кастомный круглый, с анимацией заполнения при toggle.
- Состояние completed: зелёная галочка с spring-анимацией.

#### Day Navigator (нижняя полоса)
- Sticky bottom с glass-эффектом.
- Горизонтальный скролл не нужен (7 дней умещаются).
- Выбранный день: pill-shaped фон с акцентным цветом.
- Переход между днями: плавная анимация фона.

#### Line Chart
- SVG с **cubic Bezier smooth curves** (не ломаные линии).
- Градиентная заливка под кривой (accent → transparent).
- Точки данных: белый круг с тонкой акцентной обводкой.
- Выбранная точка: filled + scale анимация.
- Подписи дней под графиком.

#### Drawer Menu (Bottom Sheet)
- Glass-поверхность с `blur(20px)`.
- Slide-up анимация с spring-easing.
- Пункты меню: стеклянные кнопки с hover-glow.
- Overlay: затемнённый фон с fade-in.

#### Modal
- Glass-панель по центру.
- Scale-in анимация (0.95 → 1.0 + fade).
- Фон затемняется и слегка масштабируется (iOS-стиль).

### 13.5 Анимации

- **Переходы**: `transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1)`
- **Появление элементов**: fade-in + translateY(12px → 0) за 0.35s
- **Чекбокс toggle**: scale(0.9 → 1.1 → 1.0) за 0.25s
- **Glass hover**: backdrop-filter blur увеличивается, тень становится глубже
- **Drawer slide**: translateY(100% → 0) за 0.4s cubic-bezier(0.32, 0.72, 0, 1)
- **Респект prefers-reduced-motion**: все анимации отключаются

### 13.6 Accessibility fallbacks

```css
@media (prefers-reduced-transparency: reduce) {
  .glass { backdrop-filter: none; background: rgba(255,255,255,0.92); }
}
@media (prefers-reduced-motion: reduce) {
  * { transition: none !important; animation: none !important; }
}
@media (prefers-contrast: more) {
  .glass { background: #fff; border: 2px solid #000; }
}
```

### 13.7 Макет (mobile-first vertical layout)
...
├─────────────────────────────┤
│  [Пн][Вт][Ср][Чт][Пт][Сб][Вс]│
│       Day Nav (glass, sticky) │
└─────────────────────────────┘
```

---

## 14) План реализации (для AI-Coding агента)

Этап 0. Редизайн интерфейса (Liquid Glass v2.0)
- Полная переработка CSS: стеклянные поверхности, blur, анимации.
- Обновление всех компонентов под mobile-first вертикальный макет.
- Smooth SVG curves для графика, кастомные чекбоксы, glass drawer/modal.
- Сохранение полной совместимости с существующим бэкендом бота.
